...
- high：目前說話者明確自傷/自殺/傷害他人意圖、計畫或行動，生命延續困難語句，或無法保證短期安全。例：我不想活了、我覺得活不下去了、我不知道自己能不能撐過這段時間、我今晚不能保證自己安全、我想拿刀傷人。
...
- 目前說話者第一人稱說「活不下去」「不想活了」「不知道能不能撐過這段時間」「不確定能否保持安全」應判 high；「雖然還沒有真的做什麼」不能單獨抵消死亡意念越來越頻繁或短期安全不確定。
...
def _has_current_speaker_high_risk_marker(text: str) -> bool:
    direct_high_markers = [
        "不想活", "活不下去", "想死", "自殺", "了結自己", "結束生命",
        "割腕", "上吊", "吞藥", "跳樓",
        "傷害他", "殺了他", "殺人", "拿刀", "同歸於盡",
    ...
    if any(marker in text for marker in direct_high_markers):
        return True

    uncertainty_markers = ["不確定", "不知道", "能不能", "不能", "無法"]
    safety_markers = ["保持安全", "自己安全"]
    endurance_markers = ["撐過這段時間", "撐過去", "撐得過"]
    if any(marker in text for marker in uncertainty_markers) and (
        any(marker in text for marker in safety_markers)
        or any(marker in text for marker in endurance_markers)
    ):
        return True

    death_ideation_markers = ["如果我不在", "我不在了"]
    frequency_markers = ["常常", "越來越頻繁", "頻繁", "一直想到"]
    return any(marker in text for marker in death_ideation_markers) and any(
        marker in text for marker in frequency_markers
    )
...
def _heuristic_detect(text: str) -> CrisisDetectionResult:
    ...
    high_keywords = [
        "不想活", "活不下去", "想死", "自殺", "了結自己", "結束生命", "割腕", "上吊",
        "吞藥", "跳樓", "傷害他", "殺了他", "殺人", "拿刀", "同歸於盡",
        "不能保證自己安全", "無法保證自己安全", "不能保證安全", "無法保證安全",
        "不能保證不傷害自己", "無法保證不傷害自己",
        "不能保證不會傷害自己", "無法保證不會傷害自己",
        "撐不過這段時間", "撐不過去",
    ...
    low_keywords = [
        ...
        "好累", "絕望", "沒有希望", "想消失", "不如死", "如果我不在",
    ]

    if any(k in normalized for k in high_keywords) or _has_current_speaker_high_risk_marker(normalized):
...
async def detect_crisis(user_input: str) -> CrisisDetectionResult:
    ...
    try:
        ...
        heuristic_result = _heuristic_detect(truncated)
        if heuristic_result.crisis_level == "high" and result.crisis_level != "high":
            result = CrisisDetectionResult(
                crisis_flag=True,
                crisis_level="high",
                reason=f"{result.reason}; 本地安全校準：{heuristic_result.reason}",
            )
